...
//add a method for checking dup customers
	...
	public static void updatePassword(String email, String oPwd, String nPwd, Map<String, Customer> customers)
			throws CustomerHandlingException {
		Customer customer = customers.get(email);
		...
		if (customer.getPassword().equals(oPwd)) {
			customer.setPassword(nPwd);
			System.out.println("new password is: " + customer.getPassword());// comment this out later.
		} else {
			throw new CustomerHandlingException("Invalid old password !!");
		}
	}

	public static void removeCustomer(String email, Map<String, Customer> customers) throws CustomerHandlingException {
