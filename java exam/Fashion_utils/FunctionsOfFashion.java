...
public class FunctionsOfFashion {
	...
	public static void removeClothes(Map<Integer, Fashion> fMap) {
		...
		Iterator<Map.Entry<Integer, Fashion>> itr = fMap.entrySet().iterator();

		while (itr.hasNext()) {

			Map.Entry<Integer, Fashion> nex = itr.next();

			Period diff = Period.between(nex.getValue().getStockUpdateDate(), now());
			int months = (diff.getYears() * 12) + diff.getMonths();
			if (months < 3) {
				System.out.println("Details of removed Clothes is:");
				System.out.println(nex.getValue());
				itr.remove();
			}
